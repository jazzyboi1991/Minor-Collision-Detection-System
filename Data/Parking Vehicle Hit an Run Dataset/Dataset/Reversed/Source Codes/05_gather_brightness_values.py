...
def gather_brightness_values():
    ...
    # Utility 폴더 안에 있으므로 부모 디렉토리가 Reversed 디렉토리임
    script_dir = os.path.dirname(os.path.abspath(__file__))
    video_dir = os.path.join(os.path.dirname(script_dir), "mp4")
    output_path = os.path.join(
        script_dir, "reversed_average_brightness_values.json")
    ...
    for video_name in tqdm(video_files, desc="Processing videos"):
        ...
        # 전체 프레임의 평균 명도 계산
        if frame_count > 0:
            # 확장자를 제외한 파일명을 키로 사용하도록 수정 (선택 사항이나 일관성을 위해)
            file_id = os.path.splitext(video_name)[0]
            brightness_data[file_id] = float(total_brightness / frame_count)
        else:
            file_id = os.path.splitext(video_name)[0]
            brightness_data[file_id] = 0.0
